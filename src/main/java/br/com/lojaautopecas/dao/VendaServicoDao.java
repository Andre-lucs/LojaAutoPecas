...
public class VendaServicoDao {
    ...
    private boolean tabelaVendaServicoExiste() throws SQLException, SQLException {
        ...
        ResultSet tables = metaData.getTables(null, null, "vendaservicodao", null);
        return tables.next();
    }
 ...
 // Método para criar uma relação Venda-Servico no banco
    public void inserirVendaServico(VendaServico vendaServico) throws SQLException {
        ...
        String sql = "INSERT INTO Venda_Servico (id_venda, id_servico) VALUES (?, ?)";
...
//        String sql = "UPDATE Venda_Servico SET id_venda=?, id_servico=? WHERE id=?";
//        try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
...
//    }
    ...
    public void deletarVendaServico(int id) {
        String sql = "DELETE FROM Venda_Servico WHERE id=?";
        try (PreparedStatement stmt = conexao.prepareStatement(sql)) {
            stmt.setInt(1, id);
            stmt.executeUpdate();
